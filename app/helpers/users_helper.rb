module UsersHelper
  ...
  def child_units(unit)
  	 _child_units = Unit.set_parent_unit(unit.id)
  end
  
  def load_users(users)
  	if users.by_user(current_user.id).present?
  		users = users.by_user(current_user.id)
  	else
  		users 
  	end
  end
  ...
  def load_users_tree(unit)
    ...
    concat (content_tag(:li, class: ["collection-item", "dismissable", "padding-r-none"], :style => "-webkit-user-drag: none;") {
      ...
      concat(content_tag(:div, :class => "col-lg-12"){
        ...
        concat(content_tag(:div, :class => "col-lg-10"){
          # concat(content_tag(:span, unit.unittype.unit_type_name.humanize, :class => "m-label teal margin-l-none margin-r-5"))
        ...
        #   })    
        # })       
        concat(content_tag(:div, class: ["col-lg-1 padding-l-r-none"]){
        })
        #Show User name email and branch
        concat(content_tag(:div, class: ["col-lg-11 padding-l-r-none"], :style => "margin-top: 0px;"){
          # concat (label_tag( unit.unit_name))
            ...
            concat (content_tag(:ul, class: ["collection-child"]){
              ...
              if @users.present?
                concat(content_tag(:table, :class => "data-table"){
                  concat(content_tag(:tr, :class => "bg-red"){
                  ...
                  @users.each do |object|
                    if User.has_parent?(object,@users) == false || current_user.id == object.id
                      ...
                      if User.has_child(object.id) == true
                        # load_child_users(object,unit)
                        concat(content_tag(:td,"-",:colspan=>12, :class => "col-lg-1"){
                          load_child_users(object,unit)
                          })
                      end
                    end
  ...
  def load_child_users(parent_user,unit)
    @child_users = parent_user.child_users.by_unit(unit.id)
    if @child_users.present?
      concat (content_tag(:li, class: ["collection-item", "dismissable", "padding-r-none"], :style => "-webkit-user-drag: none;") {
        concat(content_tag(:div, :class => "col-lg-12"){
          concat(content_tag(:div, class: ["col-lg-1 padding-l-r-none"]){
          })
          concat(content_tag(:div, class: ["col-lg-11 padding-l-r-none"], :style => "margin-top: -24px;"){
            concat(content_tag(:div, class: ["clearfix"]){})
            concat (content_tag(:p, class: ["margin-t-b-0"]){
              concat (content_tag(:ul, class: ["collection-child"]){
                concat(content_tag(:br))
                # @child_users = parent_user.child_users.by_unit(unit.id)
                concat(content_tag(:table, :class => "data-table"){
                  concat(content_tag(:tr, :class => "bg-green"){
                    # concat(content_tag(:th, "ID", :class => "col-lg-1"))
                    # concat(content_tag(:th, "Parent User", :class => "col-lg-2"))
                    # concat(content_tag(:th, "Email", :class => "col-lg-2"))
                    # concat(content_tag(:th, "UserName",:class => "col-lg-1"))
                ...
                concat(content_tag(:div, class: ["clearfix"]){})
                #Recursive call to get the child users
                # concat (content_tag(:li, class: ["collection-child"]) {
                # concat(content_tag(:br))
                @child_users.each do |child_user|
                  load_child_users(child_user,unit)
                end
                # })
              })
            })
