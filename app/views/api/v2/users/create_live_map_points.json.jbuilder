...
json.messages do
  json.simple_message @error.present? ? "Error to save live map data." : "Save live map data successfully."
  json.validation_msg @validation_msg if @validation_msg.present?
  json.internal_message "Save live map data successfully."
...
if @error.present? or @validation_errors.present?
  ...
  json.data do
    json.live_map_data @live_map_data
  end  
end