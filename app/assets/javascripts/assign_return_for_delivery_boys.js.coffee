...
$(document).ready -> 
  ...
  $('#submit_assign_order').click ->
    ...
    request = $.ajax(
        ...
        url       : "/delivery_boys/create_assign_return?data=#{orders_to_db}"
      )
  ...
  success_on_cancel_return =  ->  
    initialize()
    returns_to_db = JSON.parse $.cookie('returns_to_db')  
    
    for k of returns_to_db
      obj_array = returns_to_db[k]
...
$(document).on 'click',"input[name='return_ids[]']", (event)->
  page_action = location.pathname.split('/').slice(-1)[0]
  if page_action != 'assign_return'
